...
I started this competition by just focusing on getting a good understanding of the dataset. The EDA is detailed and many visualizations are included. This version also includes modeling. From the glmnet family of models, lasso performs best with regards to both the cross validation scores and public scores. The second models that I composed is an XGBoost model. Taking the average of those 2 models gives me the best public score at the moment.

I will continu to work on this analysis, and eventually also want to include stacking of models. If you like what I have done so far, upvotes are always appreciated :).
...
```{r}
#Getting rid of the IDs but keeping the test IDs in a vector. They are needed to compose the submission file
test_labels <- test$Id
test$Id <- NULL
train$Id <- NULL
```
...
Month Sold is also an Integer variable. However, December is not "better" than January. Therefore, I will convert MoSold values back to factors.
...
Although possible a bit less steep than expected, the effects of the Banking crises that took place at the end of 2007 can be seen indeed. After the highest median prices in 2007, the prices gradually decreased.
...
Although the correlations are giving a good overview of the most important numeric variables and multicolinerity among those variables, I wanted to get an overview of the most important variables including the categorical variables before move on to visualization.
...
Only 3 of those most important variables are categorical according to RF; Neighborhood, MSSubClass, and GarageType. Since I have already visualized The GrLivArea and OverallQuality in my initial explorations, I will not visualize those predictors again in this section.
...
GrLivArea seemed to be just the total of square feet 1st and 2nd floor. However, for instance square feet 2nd floor might be worth less than square feet 1st floor (cheaper to build, having to climb stairs, less usable space if walls aren't straight). In this case, it seems better to take only take the separate variables for prediction. However, MSSubClass also takes the number of stories into account. It will be interesting to see how for instance lasso regression deals with this group of variables.

In a later version, I discovered that there is also a variable called: LowQualFinSF: Low quality finished square feet (all floors). As you can see above (Low quality square feet 1st and 2nd) almost all houses have none of this (only 40 houses do have some). It turns out that these square feet are actually included in the GrLivArea. The correlation between those 3 variables and GrLivArea is exactely 1.
...
The size of the Basement is also ranked as an important variable. Similar to 2nd floor surface, I suspect that basement square feet might also be worth less (no windows, also needing stairs climbing/decending).
...
I have already visualized the relation between Overall Quality and SalePrice in my initial explorations, I want to vizualize the frequency distribution as well. As there are more quality measurements, I am taking the opportunity to bundle them in this section.
...
As already mentioned in section 4.2, GarageCars and GarageArea are highly correlated. Here, GarageQual and GarageCond also seem highly correlated. However, both are dominated by level =3. Therefore, I expect both of them to be dropped by algorthms that use regularization.
...
This is just a start. I only composed the most obvious ones for the time being. I intend to look into consolidating more variables (such as all Quality and Condition variables) later on, and will also see if binning more variables improves scores.
...
As you can see,the skew if now quite low and the Q-Q plot is also looking much better.
...
Please be aware that this model is a draft model. I have done done full hyperparameter tuning yet. In this version, I have only decreased the eta and left all other parameters at their defaults for the time being. I will do full hyperparameter tuning with the `mlr` package in the next version. 
...
Below I am doing cross validation to determine the best number of rounds (for the set of parameters above). 
...
Since the lasso and XGBoost algorithms are very different, averaging predictions likely improves the scores. As the lasso model does better regarding the cross validated RMSE score (0.1137 versus 0.1225), I am weigting the lasso model double.
...
In upcoming versions I will further optimize by more features engineering, and hyper parameter tuning.
